...
st.set_page_config(page_title="총괄생산계획 최적화", page_icon="🏭", layout="wide")
...
def solve_app(demand, params, model_type="LP"):
    ...
    reg_prod = params["hours"] * params["days"] / params["std_time"]
    reg_labor_cost = params["reg_wage"] * params["hours"] * params["days"]
    ...
    m.Cost = Objective(expr=sum(
        reg_labor_cost * m.W[t] + params["ot_wage"] * m.O[t] +
        params["hire_cost"] * m.H[t] + params["fire_cost"] * m.L[t] +
        params["inv_cost"] * m.I[t] + params["back_cost"] * m.S[t] +
        params["mat_cost"] * m.P[t] + params["sub_cost"] * m.C[t]
        for t in T), sense=minimize)

    m.labor = Constraint(T, rule=lambda m, t: m.W[t] == m.W[t-1] + m.H[t] - m.L[t])
    m.capacity = Constraint(T, rule=lambda m, t: m.P[t] <= reg_prod * m.W[t] + m.O[t] / params["std_time"])
    m.inventory = Constraint(T, rule=lambda m, t:
        m.I[t] == m.I[t-1] + m.P[t] + m.C[t] - demand[t-1] - m.S[t-1] + m.S[t])
    m.overtime = Constraint(T, rule=lambda m, t: m.O[t] <= params["ot_max"] * m.W[t])
    m.W_0 = Constraint(rule=m.W[0] == params["W0"])
    m.I_0 = Constraint(rule=m.I[0] == params["I0"])
    m.S_0 = Constraint(rule=m.S[0] == 0)
    m.last_inv = Constraint(rule=m.I[TH] >= params["If_min"])
    m.last_shortage = Constraint(rule=m.S[TH] == 0)

    result = SolverFactory("glpk").solve(m, tee=False)
    ...
    monthly = [{"월": f"{t}월",
                ...
                "수요(개)": demand[t-1]} for t in T]

    cb = {f"{t}월": {
        "정규노동비": reg_labor_cost * value(m.W[t]),
        "초과근무비": params["ot_wage"] * value(m.O[t]),
        "고용비": params["hire_cost"] * value(m.H[t]),
        "해고비": params["fire_cost"] * value(m.L[t]),
        "재고유지비": params["inv_cost"] * value(m.I[t]),
        "부재고비": params["back_cost"] * value(m.S[t]),
        "재료비": params["mat_cost"] * value(m.P[t]),
        "하청비": params["sub_cost"] * value(m.C[t]),
    } for t in T}

    return {"total_cost": value(m.Cost), "monthly": monthly, "cost_breakdown": cb}


with st.sidebar:
    st.markdown("## 🏭 총괄생산계획")
    st.caption("원예장비 제조업체 · Pyomo LP/IP")
    st.divider()
    ...
    st.divider()
    model_type  = st.radio("모델 유형", ["LP (실수해)", "IP (정수해)"])
...
params = dict(reg_wage=reg_wage, ot_wage=ot_wage, hire_cost=hire_cost, fire_cost=fire_cost,
              ...
              W0=W0, I0=I0, If_min=If_min, std_time=std_time, ot_max=ot_max, days=days, hours=hours)
...
with tab1:
    ...
    c2.metric("⚠️ 총 부재고",   f"{df['부재고(개)'].sum():,.0f}개",
              "없음 ✅" if df["부재고(개)"].sum()==0 else f"{int((df['부재고(개)']>0).sum())}개월 발생")
    c3.metric("👥 인력 변동",   f"+{df['고용(명)'].sum():.0f} / -{df['해고(명)'].sum():.0f}명")
    c4.metric("📦 평균 재고",   f"{df['재고(개)'].mean():,.0f}개")
    st.markdown("---")
    ...
    with col2:
        ...
        fig2.add_trace(go.Scatter(name="초과시간(h)", x=months, y=df["초과시간(h)"].tolist(),
                                  mode="lines+markers", line=dict(color="#8b5cf6", width=2)), secondary_y=True)
...
with tab2:
    ...
    for i, col in enumerate(cost_df.columns):
        fig_c.add_trace(go.Bar(name=col, x=months, y=cost_df[col].tolist(), marker_color=COLORS[i]))
    fig_c.update_layout(barmode="stack", title="월별 비용 구성 (천원)", height=380,
                        plot_bgcolor="white", legend=dict(orientation="h", y=-0.25))
    ...
    fig_cum.add_trace(go.Scatter(name="누적(천원)", x=months, y=cum_costs,
                                 mode="lines+markers", line=dict(color="#22c55e", width=2.5)), secondary_y=True)
...
with tab3:
    if result_lp is None or result_ip is None:
        ...
        lp_cost, ip_cost = result_lp["total_cost"], result_ip["total_cost"]
        ...
        c3.metric("Integrality Gap",     f"{diff/1000:,.0f}천원",    f"+{diff/lp_cost*100:.2f}%")
        st.info(f"💡 LP는 소수점 인원을 허용해 **{lp_cost/1e6:.2f}백만원** 달성. "
                f"IP는 정수 인원으로 **{ip_cost/1e6:.2f}백만원** 필요. 차이 **{diff/1000:,.0f}천원**이 정수화 비용입니다.")
        ...
        compare_data = {
            "항목": ["총 비용(천원)","평균 인원(명)","총 고용(명)","총 해고(명)","평균 재고(개)","총 부재고(개)","총 하청(개)"],
            "LP (실수해)": [f"{lp_cost/1000:,.0f}", f"{df_lp['인원(명)'].mean():.2f}",
                            f"{df_lp['고용(명)'].sum():.2f}", f"{df_lp['해고(명)'].sum():.2f}",
                            f"{df_lp['재고(개)'].mean():.0f}", f"{df_lp['부재고(개)'].sum():.0f}",
                            f"{df_lp['하청(개)'].sum():.0f}"],
            "IP (정수해)": [f"{ip_cost/1000:,.0f}", f"{df_ip['인원(명)'].mean():.1f}",
...
with tab4:
    ...
    st.caption("슬라이더를 조정하면 비용 구성 변화를 즉시 시뮬레이션합니다")
    ...
    with col2:
        orig = {k: sum(cb[f"{t+1}월"][k] for t in range(6)) for k in cost_keys}
        mults = {"정규노동비":1,"초과근무비":ot_m,"고용비":hire_m,"해고비":fire_m,
                 "재고유지비":inv_m,"부재고비":back_m,"재료비":1,"하청비":1}
        new = {k: orig[k]*mults[k] for k in orig}
        orig_total, new_total = sum(orig.values()), sum(new.values())
        ...
        fig_s = go.Figure()
        fig_s.add_trace(go.Bar(name="기존",    x=cost_keys,
                               y=[orig[k]/1000 for k in cost_keys],
                               marker_color=COLORS, opacity=0.4))
        fig_s.add_trace(go.Bar(name="조정 후", x=cost_keys,
                               y=[new[k]/1000 for k in cost_keys],
...
with tab5:
    ...
    for i, row in df.iterrows():
        ...
        if row["초과시간(h)"] > ot_max * row["인원(명)"] * 0.85:
            alerts.append(("warning", f"⚠️ {mo}: 초과근무 한도({ot_max}h)의 85% 초과"))
        if util > 90:
    ...
    col1, col2 = st.columns(2)
    with col1:
        clr_u = ["#ef4444" if u>90 else "#f59e0b" if u>75 else "#22c55e" for u in util_data]
        fig_u = go.Figure(go.Bar(x=months, y=util_data, marker_color=clr_u,
                                 text=[f"{u:.0f}%" for u in util_data], textposition="outside"))
    ...
    with col2:
        ...
        for i, row in df.iterrows():
            ...
            status = "🔴 부재고" if row["부재고(개)"]>0 else "🟡 주의" if util>75 else "🟢 정상"
            status_rows.append({"월": months[i], "인원(명)": row["인원(명)"],
                                 "생산(개)": row["생산(개)"], "가동률(%)": util, "상태": status})
